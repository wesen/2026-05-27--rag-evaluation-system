...
OUT_DIR="$TICKET_DIR/sources/visual-parity/package-story-sweep"
STORYBOOK_DIR="$OUT_DIR/storybook-static"
REPORTS_DIR="$OUT_DIR/reports"
PORT="${CSSVD_PACKAGE_STORYBOOK_PORT:-18617}"
...
jq -r '.entries | to_entries[] | select(.value.type == "story") | [.key, .value.title, .value.name] | @tsv' "$INDEX_JSON" > "$STORY_LIST"
  ...
  if [[ -f "$report_md" ]]; then
    {
      cat <<FM
---
Title: Package Storybook Visual Sweep - ${title} - ${name}
...
RelatedFiles: []
ExternalSources: []
Summary: css-visual-diff self-compare baseline for package Storybook story ${story_id}.
...
printf 'Package Storybook visual sweep complete.\nStories: %s\nSummary: %s\nReports: %s\n' "$(($(wc -l < "$STORY_LIST")))" "$SUMMARY" "$REPORTS_DIR"
